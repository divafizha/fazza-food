  ...
  <aside class="w-52 border-r border-gray-300 flex flex-col p-4 bg-white sticky top-0 h-screen">
    <div class="mb-10 flex justify-center">
      <?php if (file_exists($logoPath)) {
        echo '<img src="' . htmlspecialchars($logoPath) . '" alt="Logo perusahaan" class="max-h-15 object-contain" />';
      } else {
    ...
    <nav class="flex flex-col space-y-3 text-sm">
      ...
      foreach ($menuItems as $key => $label) {
        $isActive = ($page === $key) ? 'bg-yellow-200 border-yellow-300 font-semibold' : 'border-gray-300';
        echo "<a href='Index.php?page=$key' class='text-center py-2 rounded border $isActive shadow-sm text-black transition-all hover:bg-yellow-100 hover:border-yellow-200'>$label</a>";
      }
      ?>
