...
$pdfUrl  = $baseUrl.'/laporan/laporan_pdf.php?kategori='.urlencode($kategori).'&periode='.urlencode($periode).'&tanggal='.urlencode($tanggal);
...
function buildRanges($periode,$start,$rows=6){
    $ranges=[];$cursor=$start;
    for($i=0;$i<$rows;$i++){
        $ranges[]=[
...
if($kategori==='semua'){
    $ranges=buildRanges($periode,$tanggal,6);
    foreach($ranges as $r){
    ...
    $data=qRows($koneksi,"SELECT tanggal,waktu_mulai,waktu_selesai,jenis_kegiatan FROM jadwal WHERE tanggal BETWEEN '{$koneksi->real_escape_string($tanggal)}' AND '{$koneksi->real_escape_string($end)}' ORDER BY tanggal ASC, waktu_mulai ASC");
}else{
    $end=periodEnd($periode,$tanggal);
    switch($kategori){
        case 'produksi':
            $sql="SELECT p.nama_produk,pr.jumlah_produksi,pr.jumlah_dikemas,pr.jumlah_reject,COALESCE(pr.tgl_produksi,j.tanggal) AS tanggal_produksi
                  FROM produksi pr JOIN produk p ON p.id_produk=pr.id_produk LEFT JOIN jadwal j ON j.id_jadwal=pr.id_jadwal
                  WHERE COALESCE(pr.tgl_produksi,j.tanggal) BETWEEN '{$koneksi->real_escape_string($tanggal)}' AND '{$koneksi->real_escape_string($end)}'
                  ORDER BY tanggal_produksi DESC, pr.id_produksi DESC";
            break;
        case 'stok':
            $sql="SELECT s.id_stok,p.nama_produk,s.jumlah_stok,s.status_stok,s.id_produksi FROM stok s JOIN produk p ON p.id_produk=s.id_produk ORDER BY s.id_stok DESC";
            break;
        case 'pekerja_lepas':
            $sql="SELECT pl.nama_pekerja,rg.tanggal,rg.berat_barang_kg,rg.tarif_per_kg,rg.total_gaji,rg.keterangan
                  FROM riwayat_gaji rg JOIN pekerja_lepas pl ON pl.id_pekerja=rg.id_pekerja
            ...
            break;
        case 'distribusi':
            $sql="SELECT d.nama_distributor,d.alamat_distributor,p.nama_produk,d.jumlah_pesanan,d.tanggal_pesanan,d.status_pengiriman
                  FROM distribusi d JOIN produk p ON p.id_produk=d.id_produk
                  WHERE d.tanggal_pesanan IS NOT NULL AND d.tanggal_pesanan BETWEEN '{$koneksi->real_escape_string($tanggal)}' AND '{$koneksi->real_escape_string($end)}'
...
<form method="GET" class="flex flex-wrap gap-4 items-end mb-6">
  ...
  <div class="flex items-center gap-2">
    <a href="<?= htmlspecialchars($pdfUrl) ?>" target="_blank" rel="noopener"
       class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded mt-1 inline-flex items-center"
